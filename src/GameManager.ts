...
export class GameManager {
  ...
  private shelfCols = 4;  // 4 columns for mobile layout
  private readonly slotWidth = 0.50;  // Larger slots for better visibility
  private readonly rowHeight = 0.45;  // Compact rows for mobile
  ...
  public loadLevel(level: Level): void {
    ...
    // Use shelf config from level (supports multiple rows)
    this.shelfCols = level.shelf.cols;
  ...
  private buildShelf(config: ShelfConfig): void {
    ...
    const shelfHeight = config.rows * this.rowHeight;  // Multi-row support
    const shelfDepth = 0.45;
    const fillFridgeLook = this.simplifiedDecorMode;

    // ── HONEY WOOD: Light honey wood texture with visible grain
    const createHoneyWoodTexture = (): THREE.CanvasTexture => {
      const canvas = document.createElement('canvas');
      canvas.width = 1024;
      canvas.height = 512;
      const ctx = canvas.getContext('2d')!;

      // Base gradient - honey light tones
      const gradient = ctx.createLinearGradient(0, 0, 0, 512);
      gradient.addColorStop(0, '#C4903D');   // Honey light
      gradient.addColorStop(0.25, '#D4A54E'); // Lighter honey
      gradient.addColorStop(0.5, '#C4903D');
      gradient.addColorStop(0.75, '#B8842F'); // Slightly deeper
      gradient.addColorStop(1, '#8B6914');   // Honey dark
      ctx.fillStyle = gradient;
      ctx.fillRect(0, 0, 1024, 512);

      // Wood grain lines - visible and prominent (180+ lines, alpha 0.25-0.30)
      ctx.globalAlpha = 0.27;
      for (let i = 0; i < 190; i++) {
        ctx.strokeStyle = i % 3 === 0 ? '#6B4A0A' : (i % 5 === 0 ? '#A07828' : '#8B6914');
        ctx.lineWidth = Math.random() * 2.2 + 0.4;
        ctx.beginPath();
        const y = Math.random() * 512;
        const waveAmplitude = Math.random() * 3.5 + 1.5;
        const waveFrequency = Math.random() * 0.025 + 0.012;
        ctx.moveTo(0, y);
        for (let x = 0; x < 1024; x += 12) {
          ctx.lineTo(x, y + Math.sin(x * waveFrequency) * waveAmplitude + Math.random() * 1.5);
        }
        ctx.stroke();
      }

      // Wood knots - warm honey tones
      ctx.globalAlpha = 0.3;
      for (let i = 0; i < 5; i++) {
        const kx = Math.random() * 900 + 62;
        const ky = Math.random() * 400 + 56;
        const kr = Math.random() * 14 + 8;
        ...
        for (let r = 3; r < 5; r++) {
          ctx.beginPath();
          ctx.ellipse(kx, ky, kr - r * 2, (kr - r * 2) * 0.6, Math.random() * Math.PI, 0, Math.PI * 2);
          ctx.stroke();
        }
      }
      ...
      for (let i = 0; i < 40; i++) {
        const vx = Math.random() * 1024;
        const vy = Math.random() * 512;
        const vr = Math.random() * 70 + 30;
        const varGrad = ctx.createRadialGradient(vx, vy, 0, vx, vy, vr);
        varGrad.addColorStop(0, '#D4A54E');
    ...
    const woodTexture = createHoneyWoodTexture();

    // ── WARM SHOP: Background wall - light warm wood paneling like mockup
    const wallGeometry = new THREE.PlaneGeometry(shelfWidth + 6, shelfHeight + 6);
    const wallCanvas = document.createElement('canvas');
    wallCanvas.width = 512;
    wallCanvas.height = 512;
    const wallCtx = wallCanvas.getContext('2d')!;
    // Light warm brown base
    wallCtx.fillStyle = '#C19A6B';
    ...
    const wallMaterial = new THREE.MeshStandardMaterial({
      color: fillFridgeLook ? 0xf2f9ff : 0xC19A6B,
      map: fillFridgeLook ? null : wallTexture,
      roughness: fillFridgeLook ? 1.0 : 0.75,
      metalness: 0.0,
    });
    ...
    // ── HONEY SHELF: Shelf backboard - honey wood
    const backGeometry = new THREE.BoxGeometry(shelfWidth + 0.2, shelfHeight + 0.2, 0.08);
    const backMaterial = new THREE.MeshStandardMaterial({
      color: fillFridgeLook ? 0xd9ebff : 0xC4903D,
      map: fillFridgeLook ? null : woodTexture,
      roughness: fillFridgeLook ? 0.95 : 0.85,
      metalness: fillFridgeLook ? 0.0 : 0.02,
      bumpMap: fillFridgeLook ? null : woodTexture,
      bumpScale: fillFridgeLook ? 0 : 0.012,
    });
    ...
    // ── HONEY SHELF: Horizontal shelf boards - honey light wood
    const shelfBoardColor = fillFridgeLook ? 0xe6f3ff : 0xC4903D;
    const goldColor = fillFridgeLook ? 0xffffff : 0xc9a227;
    // Generate board positions for all rows (bottom, middle(s), top)
    const boardPositions: number[] = [];
    for (let i = 0; i <= config.rows; i++) {
      const y = -shelfHeight / 2 + i * this.rowHeight;
      boardPositions.push(y);
    }
    ...
    // ── HONEY SHELF: Vertical dividers - dark honey edges
    const dividerColor = fillFridgeLook ? 0xc6def8 : 0x8B6914;
    for (let col = 0; col <= config.cols; col++) {
      const x = (col - config.cols / 2) * slotWidth;
      // Wood divider - dark honey edge
      const dividerGeometry = new THREE.BoxGeometry(0.06, shelfHeight, shelfDepth * 0.9);
      const dividerMaterial = new THREE.MeshStandardMaterial({
        color: dividerColor,
        map: fillFridgeLook ? null : woodTexture,
        roughness: fillFridgeLook ? 0.96 : 0.9,
    ...
    // ── HONEY SHELF: Side walls - honey wood with visible grain
    ...
    // Position shelf centered for mobile portrait view
    this.shelfGroup.position.y = 0.4;  // Lower for better framing
  ...
  private createVinyls(vinyls: Vinyl[]): void {
    ...
    // ── Carousel platform - lower for mobile portrait
    const barWidth = 8;
    const barHeight = 0.28;  // Slimmer bar
    const barDepth = 0.28;
    const carouselBaseY = -1.2;  // Much lower
    const vinylBaseY = -0.70;  // Much lower for vinyls
    ...
    carouselBar.position.set(0, carouselBaseY, 2.3);
    ...
    accent.position.set(0, carouselBaseY + barHeight / 2 + 0.015, 2.3);
    ...
    glowLine.position.set(0, carouselBaseY + barHeight / 2 - 0.02, 2.3 + barDepth / 2 + 0.012);
    ...
    if (this.aiPhotoDecalsEnabled) {
      ...
      counterDecal.position.set(0, carouselBaseY, 2.3 + barDepth / 2 + 0.011);
    ...
    // ── Vinyl records in carousel - smaller and more compact for mobile
    const spacing = 0.42;  // Tighter spacing
    vinyls.forEach((vinyl, index) => {
      const vinylMesh = this.createVinylMesh(vinyl);
      vinylMesh.scale.set(0.75, 0.75, 0.75);  // Smaller vinyls (75% of original)
      vinylMesh.userData = {
        vinyl,
        carouselIndex: index,
      ...
      vinylMesh.scale.set(1.2, 1.2, 1.2);
      vinylMesh.rotation.set(-0.05, 0, 0);
      vinylMesh.frustumCulled = false;
    ...
    const animateArrow = () => {
      time += 0.04;
      arrow.position.y = 0.12 + Math.sin(time) * 0.07;
  ...
  private createVinylMesh(vinyl: Vinyl): THREE.Group {
    ...
    const vinylWidth = 0.42;
    const vinylHeight = 0.42;
    const vinylThickness = 0.01;
    const vinylColorObj = new THREE.Color(vinyl.color);

    // ── Cardboard sleeve (genre color for visibility against cream scene)
    const sleeveGeometry = new THREE.BoxGeometry(vinylWidth, vinylHeight, vinylThickness);
    const sleeveMaterial = new THREE.MeshStandardMaterial({
      color: vinylColorObj.clone().lerp(new THREE.Color(0xffffff), 0.3),
      roughness: 0.5,
      metalness: 0.0,
      emissive: vinylColorObj,
    ...
    const discRadius = vinylWidth * 0.35;
    ...
    discCtx.beginPath();
    discCtx.arc(512, 512, 512, 0, Math.PI * 2);
    discCtx.fill();

    // Realistic grooves with varied thickness
    for (let r = 132; r < 484; r += 10) {
      discCtx.strokeStyle = r % 30 === 0 ? 'rgba(60,60,60,0.5)' : 'rgba(40,40,40,0.3)';
      discCtx.lineWidth = r % 30 === 0 ? 1.5 : 0.8;
      discCtx.beginPath();
      discCtx.arc(512, 512, r, 0, Math.PI * 2);
      discCtx.stroke();
    }

    // Realistic light reflection arc
    const arcGrad = discCtx.createLinearGradient(200, 200, 700, 500);
    arcGrad.addColorStop(0, 'rgba(255,255,255,0.0)');
    arcGrad.addColorStop(0.3, 'rgba(255,255,255,0.25)');
    arcGrad.addColorStop(0.7, 'rgba(255,255,255,0.25)');
    arcGrad.addColorStop(1, 'rgba(255,255,255,0.0)');
    discCtx.strokeStyle = arcGrad;
    discCtx.lineWidth = 40;
    discCtx.beginPath();
    discCtx.arc(512, 512, 320, -Math.PI * 0.4, Math.PI * 0.12);
    discCtx.stroke();

    // Center label with genre color
    discCtx.fillStyle = vinyl.color;
    discCtx.beginPath();
    discCtx.arc(512, 512, 120, 0, Math.PI * 2);
    discCtx.fill();
    ...
    discCtx.lineWidth = 2;
    discCtx.beginPath();
    discCtx.arc(512, 512, 110, 0, Math.PI * 2);
    discCtx.stroke();
    discCtx.beginPath();
    discCtx.arc(512, 512, 90, 0, Math.PI * 2);
    discCtx.stroke();

    // Center hole
    discCtx.fillStyle = '#000';
    discCtx.beginPath();
    discCtx.arc(512, 512, 16, 0, Math.PI * 2);
    discCtx.fill();

    // Rainbow iridescent reflection on grooves
    const iridGrad = discCtx.createConicGradient(0, 512, 512);
    iridGrad.addColorStop(0, 'rgba(255, 100, 100, 0.06)');
    iridGrad.addColorStop(0.15, 'rgba(100, 255, 100, 0.06)');
    iridGrad.addColorStop(0.3, 'rgba(100, 100, 255, 0.06)');
    iridGrad.addColorStop(0.45, 'rgba(255, 255, 100, 0.06)');
    iridGrad.addColorStop(0.6, 'rgba(255, 100, 255, 0.06)');
    iridGrad.addColorStop(0.75, 'rgba(100, 255, 255, 0.06)');
    iridGrad.addColorStop(1, 'rgba(255, 100, 100, 0.06)');
    discCtx.fillStyle = iridGrad;
    discCtx.beginPath();
    discCtx.arc(512, 512, 484, 0, Math.PI * 2);
    discCtx.arc(512, 512, 132, 0, Math.PI * 2, true);
    discCtx.fill();

    // Label text
    discCtx.fillStyle = '#fff';
    discCtx.font = 'bold 32px Arial';
    discCtx.textAlign = 'center';
    discCtx.fillText(vinyl.genre.toUpperCase(), 512, 500);
    discCtx.font = '22px Arial';
    discCtx.fillText('SLEEVO', 512, 540);
    ...
    disc.position.set(vinylWidth * 0.40, 0, -vinylThickness / 2 - 0.005);
  ...
  public update(deltaTime: number): void {
    ...
    const scrollSpeed = 0.35 * speedMultiplier;
    ...
    this.vinylsGroup.children.forEach((child) => {
      if (child.name.startsWith('vinyl-')) {
        ...
        // Carousel wrapping: keep vinyls cycling within visible range
        const carouselSpacing = 0.42;  // Match creation spacing
        const totalWidth = carouselVinylCount * carouselSpacing;
        const wrapMin = -totalWidth / 2 - carouselSpacing;
        if (child.position.x < wrapMin) {
          child.position.x += totalWidth + carouselSpacing;
        }

        // Center-based focus for scale and depth effects - subtle for mobile
        const centerFocus = Math.max(0, 1 - Math.min(Math.abs(child.position.x) / 3.4, 1));
        const baseScale = 0.75;  // Smaller base scale
        const targetScale = baseScale + centerFocus * 0.12;  // Subtle scale change
        child.scale.lerp(new THREE.Vector3(targetScale, targetScale, targetScale), 0.12);

        child.rotation.y = 0;

        const targetZ = 2.15 + centerFocus * 0.12;
        ...
        const baseY = typeof userData.vinylBaseY === 'number' ? userData.vinylBaseY : -0.10;
  ...
  public placeVinylOnShelf(vinyl: THREE.Object3D, row: number, col: number): void {
    // Use actual row parameter instead of forcing to 0
    ...
    vinyl.scale.set(1.15, 1.15, 1.15);  // Larger for better visibility
  ...
  public ensureSpineVisible(vinyl: THREE.Object3D): void {
    ...
    const vinylWidth = 0.25;
    const vinylHeight = 0.30;
  ...
  private getShelfCellCenter(row: number, col: number): THREE.Vector3 {
    // Multi-row support: calculate x and y based on row and col
    const x = (col - this.shelfCols / 2 + 0.5) * this.slotWidth;
    // Calculate Y: rows are from bottom to top
    const shelfHeight = this.level!.shelf.rows * this.rowHeight;
    const y = -shelfHeight / 2 + (row + 0.5) * this.rowHeight;
