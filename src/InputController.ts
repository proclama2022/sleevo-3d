...
export class InputController {
  ...
  public setShelfConfig(cols: number, slotWidth?: number, rowHeight?: number): void {
    ...
    console.log(`Shelf config updated: ${this.shelfConfig.rows} row x ${this.shelfConfig.cols} cols`);
  }
    ...
    if (this.isDragging && this.selectedVinyl) {
      ...
      if (canPlace) {
        ...
        if (snapped && !isGenreMatch) {
          const targetLabel = expectedCol !== null ? getColumnLabel(expectedCol) : 'corretta';
          console.log(`❌ Wrong genre column. Drop ${vinylGenre} in ${targetLabel}.`);
        } else if (snapped && isGenreMatch && !isExactColumn) {
          const targetLabel = expectedCol !== null ? getColumnLabel(expectedCol) : 'corretta';
          console.log(`❌ Wrong slot. Put this vinyl in ${targetLabel}.`);
        } else if (snapped && !isSlotFree) {
          console.log('❌ Slot already occupied. Choose an empty column.');
        } else {
  ...
  private snapToShelfGrid(position: THREE.Vector3): { row: number, col: number, snapped: boolean } {
    ...
    console.log('Snap check:', `x=${x.toFixed(2)}, y=${y.toFixed(2)}, projected=${!!projected}`);
    ...
    console.log('✅ Within bounds, will snap to shelf');
